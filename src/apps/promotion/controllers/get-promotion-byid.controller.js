...
export const GetPromotionById = async (req, res) => {
  try {
    ...
    const promotion = await PromotionModel.findById(id)
      .populate({
        path: 'campaign',
        select: 'title mediaUrl caption link category mediaType budget payoutPerPromotion costPerClick currency campaignType priority difficulty tags thumbnailUrl estimatedViews duration targetAudience requirements activityLog createdAt endDate status spentBudget remainingBudget campaignGoal',
        populate: {
          path: 'owner',
