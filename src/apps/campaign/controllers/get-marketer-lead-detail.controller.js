...
export const getMarketerLeadDetail = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(marketerId) || !mongoose.Types.ObjectId.isValid(campaignId)) {
    ...
    const campaign = await CampaignModel.findOne({ _id: campaignOid, owner: marketerOid })
      .select('title status')
      .lean();

    if (!campaign) {
      return res.status(404).json({ success: false, message: 'Campaign not found or not owned by you.' });
    }
    ...
    console.error('Get marketer lead detail error:', error);
